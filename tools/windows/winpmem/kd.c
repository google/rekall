...
void *KernelGetProcAddress(void *image_base, char *func_name) {
	...
	((uintptr_t)image_base + expdir->VirtualAddress);
      ...
      unsigned int *functions = (uintptr_t)image_base + exports->AddressOfFunctions;
      unsigned int *names = (uintptr_t)image_base + exports->AddressOfNames;

      short *ordinals = (uintptr_t)image_base + exports->AddressOfNameOrdinals;
...
KDDEBUGGER_DATA64 *KDBGScan(IMAGE_DOS_HEADER *image_base) {
  ...
  if (data_section) {
    char *buffer = data_section->VirtualAddress + (uintptr_t)image_base;
...
int GetKPCR(struct PmemMemroyInfo *info) {
  int number_of_processors = min(32, KeQueryActiveProcessors());
  int i;
  ...
  for (i=0; i<number_of_processors-1; i++) {
    ...
    info->KPCR[i].QuadPart = (uintptr_t)__readfsqword(0x1c);
#endif
  };
